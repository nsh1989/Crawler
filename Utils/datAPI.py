...
class cdatapi:
    ...
    def getToken(self, customerNumber, customerLogin, customerPassword, interfacePartnerNumber, interfacePartnerSignature):
        token_param = {
            'request': {
        ...
        client = Client(wsdl=self.token_wsdl)
        token = client.service.generateToken(**token_param)
    ...
    def MakeSessionID(self, customerNumber, customerLogin, interfacePartnerNumber, interfacePartnerSignature,
                      customerSignature):
        client = Client(wsdl=self.token_wsdl)
        login_data = {
            'request': {
        ...
        sessionID = client.service.doLogin(**login_data)
